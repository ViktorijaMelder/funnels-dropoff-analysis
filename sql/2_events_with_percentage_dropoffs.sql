WITH ranked_events AS (
  ...
  SELECT 
    event_name, 
    event_count,
    100.0 * event_count / MAX(CASE WHEN event_name = 'session_start' THEN event_count END) OVER () AS event_percentage /*using MAX for non 'session_start' rows not to get NULL values. Using OVER() so that all rows would be divided by 'session_start' counts*/
  FROM event_counts
  ),
    final AS (
  SELECT 
    event_name, 
    event_count, 
    LAG(event_count) OVER (ORDER BY event_percentage DESC) AS previous_event_count,/*using LAG function to get previous event count for later drop off percentage calculations*/
